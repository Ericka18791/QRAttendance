...
> ✅ **STOP AND THINK:** In your own words, why is having one `studentId` for everyone a problem? Having the same studentId for everyone is a problem because the app cannot tell students apart.
...
> ✅ **STOP AND THINK:** What would happen if a user signed up but the trigger didn't exist? If the trigger didn't exist, the user would be created in auth.users but no corresponding profile would be created in profiles. When the app tries to read the user's profile, it would find no profile data, which could cause missing information or problems in parts of the app that depend on the profile.  
...
We need to JOIN the attendance and events tables. The attendance table tells us which student attended and when, while the events table provides the details of the event. They are connected through attendance.event_id and events.id.
---
